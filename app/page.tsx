                    ...
                    <div className="hidden md:flex space-x-6 text-sm text-zinc-400">
                        {sections.map((s) => (
                            <a
                                key={s.id}
                                href={`#${s.id}`}
                                className={`transition-colors duration-200 hover:text-white ${activeSection === s.id ? 'text-indigo-500 font-semibold' : ''
                                    }`}
                            >
                                {s.name}
                            </a>
                ...
                <AnimatePresence>
                    {menuOpen && (
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="md:hidden bg-black/95  flex flex-col items-end  pr-8"
                        >
                            ...
                            {sections.map((s) => (
                                <a
                                    ...
                                    className={`text-white text-sm mb-4 hover:text-indigo-500 transition-colors duration-200 ${activeSection === s.id ? 'text-indigo-500 font-semibold' : ''
                                        }`}
                                >
                                    {s.name}
                                </a>
